  ...
  async function loadStatus() {
    ...
    try {
      ...
      const rows = [
        ...
        ...(d.payments.verificacion ? [['Verificación del webhook', d.payments.verificacion.code
          ? `<code class="admin-code">${esc(d.payments.verificacion.code)}</code> <span class="admin-muted admin-small">recibido ${fmtDT(d.payments.verificacion.at)} · cópialo en Openpay para confirmar el webhook.</span>`
          : `${pill(false, '', 'Llegó sin código', true)} <span class="admin-muted admin-small">${fmtDT(d.payments.verificacion.at)} · busca el aviso completo en Intentos bloqueados.</span>`]] : []),
