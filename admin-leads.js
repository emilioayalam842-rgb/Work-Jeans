...
(function () {
  ...
  // Los estados se construyen como elementos, no como texto insertado en la página.
  function rellenarEstados(raiz) {
    (raiz || document).querySelectorAll('select[data-llenar-estado]:not([data-lleno])').forEach((sel) => {
      sel.dataset.lleno = '1';
      llenarSelect(sel, Object.entries(STATUS).map(([k, v]) => crearOpcion(k, v, k === sel.dataset.llenarEstado)));
    });
  }
  new MutationObserver(() => rellenarEstados()).observe(document.body, { childList: true, subtree: true });
  ...
  function renderLeads() {
    ...
    document.getElementById('leadsTableBody').innerHTML = rows.length ? rows.map((l) => `
      <tr data-id="${esc(l.id)}">
        ...
        <td><select class="admin-status-select lead-${esc(l.status)}" data-action="lead-status" data-llenar-estado="${esc(l.status)}" aria-label="Estado de la cotización de ${esc(l.company || l.name || '')}"></select></td>
  ...
  function openLeadDetail(lead) {
    ...
    ov.innerHTML = `
      <div class="admin-form admin-form--720" id="leadDetailBox">
        ...
        <select id="leadDetailStatus" class="admin-filter" data-llenar-estado="${esc(lead.status)}" aria-label="Estado de la cotización"></select>
